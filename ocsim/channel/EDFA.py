...
class EDFA:
    ...
    def noise_sequence(self,signal):
        ...
        def noise_sequence_real(backend):
            ...
            noise_sequence = backend.sqrt(noise_power/2) * (backend.random.randn(*signal.shape)+1j * backend.random.randn(*signal.shape))
...
class ConstantGainEDFA(EDFA):
    ...
    def __core(self,signal):
        ...
        def core_real(backend):
            ...
            signal[:] = signal[:] * backend.sqrt(self.gain_linear)*signal[:]
