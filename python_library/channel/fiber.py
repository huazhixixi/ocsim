...
class Fiber:
    ...
    def prop(self, signal):
        ...
        self.device_number = signal.device_number
        self.backend = signal.backend


class NonlinearFiber(Fiber):

    def __init__(self, fiber_setting):
        super(NonlinearFiber, self).__init__(fiber_setting)
    ...
    def prop(self, signal):
        ...
        wave_length = c / signal.center_frequency
...
def nonlinear_prop(fiber: NonlinearFiber, signal):
    ...
    def nonlinear_prop_():
        if 'cuda' in signal.device:
            import cupy as cp
            import cupyx as cu
            fiber.fft_backend = cu.scipy
            fiber.backend = cp
            plan = cu.scipy.fftpack.get_fft_plan(signal[0])
            with plan:
            ...
            import numpy as np
            fiber.backend = np
            fiber.fft_backend = scipy
            return fiber.prop(signal=signal)

    return nonlinear_prop_()
